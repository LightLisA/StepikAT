...
class TestStepik(unittest.TestCase):
	...
	def test_form_2(self):
		...
		f2_lastname = f2_browser.find_element_by_css_selector(".first_block .second")
		f2_lastname.send_keys("Willyams")

		f2_email = find_element_by_css_selector(".first_block .third")
		f2_email.send_keys("rosa.wil@cogery.com")

		# Заполняет не обязательные поля
		f2_phone = find_element_by_css_selector(".second_block .first")
		f2_phone.send_keys("+875456212001")

		f2_address = find_element_by_css_selector(".second_block .second")
		f2_address.send_keys("42 SYSCON ROAD WILLYAMS")

		# НАжимаем Отправить форму
		f2_button = f2_browser.find_element_by_css_selector("button.btn")
		f2_button.click()
		...
		# находим элемент, содержащий текст
		f2_text_elm = f2_browser.find_element_by_css_selector(".container h1")
